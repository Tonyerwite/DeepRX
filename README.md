...
The figure below was generated by this repository from a 30,000-step PyTorch checkpoint and a fresh Monte Carlo evaluation with 100 samples for every SNR and pilot configuration.

![DeepRx Fig. 6(a) reproduction](docs/assets/figure6a_30k_mc100.png)

Configuration:

- Training: 30,000 steps, 8 frames per step, 10 TTIs per frame, seed 2026.
- Evaluation: SINR points <code>0:3:21</code> dB, 100 Monte Carlo samples per point and pilot configuration, 1 frame per sample, seed 2026.
...
- The single known-channel curve averages both pilot groups, for 200 receiver evaluations per SNR point.

| SINR (dB) | DeepRx, 1 pilot | DeepRx, 2 pilots | LMMSE, 1 pilot | LMMSE, 2 pilots | LMMSE, known channel |
|---:|---:|---:|---:|---:|---:|
| 0  | 0.198101 | 0.165811 | 0.266905 | 0.220959 | 0.174517 |
| 3  | 0.116569 | 0.100163 | 0.223694 | 0.119599 | 0.105644 |
| 6  | 0.053265 | 0.046135 | 0.188211 | 0.059100 | 0.048796 |
| 9  | 0.016957 | 0.013924 | 0.165639 | 0.022528 | 0.015183 |
| 12 | 0.004064 | 0.002817 | 0.154496 | 0.007657 | 0.003289 |
| 15 | 0.001173 | 0.000591 | 0.150231 | 0.003755 | 0.000727 |
| 18 | 0.000539 | 0.000188 | 0.148221 | 0.002431 | 0.000233 |
| 21 | 0.000356 | 0.000096 | 0.147172 | 0.001847 | 0.000110 |

The machine-readable values are stored in [docs/assets/figure6a_30k_mc100_metrics.json](docs/assets/figure6a_30k_mc100_metrics.json).
...
### 4. Run the delivered MC=100 Fig. 6(a) evaluation

~~~powershell
.\.venv\Scripts\python.exe scripts\reproduce_figure6a.py --checkpoint checkpoints\deeprx_official_matlab_state_dict.pth --samples-per-point 100 --n-frames 1 --seed 2026 --snr-points "0,3,6,9,12,15,18,21" --output-dir outputs\figure6a_30k_mc100
~~~
...
- Evaluation uses the physical two-RX waveform, realized per-antenna signal power for complex AWGN, and an independent seeded channel realization for every Monte Carlo sample.
- DeepRx input features are mapped to the fixed checkpoint's training scale before inference; both LMMSE receivers operate on the physical received waveform.
...
- The result is a fresh protocol-aligned reproduction. It is not a digitized or bit-identical copy of the paper figure because the authors' validation seed, exact frame order, and original MATLAB release are not public.
...
The current suite contains 32 tests. The delivered Fig. 6(a) result contains exactly eight finite values in every curve, <code>samples_per_point=100</code>, <code>n_frames=1</code>, a valid JSON file, and a readable PNG.
